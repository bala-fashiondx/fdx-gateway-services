import {inject, CoreBindings, ApplicationConfig} from '@loopback/core';
...
export class Graceful {
  // Map of all open connections on the server
  connections: {[key: string]: Socket};
  constructor(
    @inject(CoreBindings.APPLICATION_INSTANCE)
    private app: Application,
    @inject('fdx.app.config') private config: ApplicationConfig,
    @inject('fdx.http.server') private server: http.Server,
  ...
  async start() {
    ...
    try {
      ...
      const response: AnyType = await this.ecommService.checkHealth();
