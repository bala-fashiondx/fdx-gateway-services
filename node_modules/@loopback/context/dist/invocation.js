...
function invokeMethod(target, method, ctx, nonInjectedArgs = [], options = {}) {
    if (options.skipParameterInjection) {
        if (options.skipInterceptors) {
            ...
            return interceptor_1.invokeMethodWithInterceptors(ctx, target, method, nonInjectedArgs);
        }
    }
    // Parameter injection is required
    const invoker = options.skipInterceptors
        ? invokeTargetMethod
        : interceptor_1.invokeMethodWithInterceptors;
 ...
 * @param nonInjectedArgs - Optional array of args for non-injected parameters
 */
function invokeMethodWithInvoker(invoker, ctx, target, method, nonInjectedArgs) {
